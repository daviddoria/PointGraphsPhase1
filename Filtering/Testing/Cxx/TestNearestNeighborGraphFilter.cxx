...
int TestNearestNeighborGraphFilter(int, char*[])
{
  // Create data set
  ...
  vtkSmartPointer<vtkNearestNeighborGraphFilter> nearestNeighborGraphFilter =
      vtkSmartPointer<vtkNearestNeighborGraphFilter>::New();
  nearestNeighborGraphFilter->SetKNeighbors(2);
  nearestNeighborGraphFilter->SetInputConnection(polydata->GetProducerPort());
  ...
  if(groundTruth->GetNumberOfVertices() != nearestNeighborGraphFilter->GetOutput()->GetNumberOfVertices())
    {
    ...
    std::cout << "nearest neighbor graph: " << nearestNeighborGraphFilter->GetOutput()->GetNumberOfVertices() << std::endl;
    return EXIT_FAILURE;
    }

  if(groundTruth->GetNumberOfEdges() != nearestNeighborGraphFilter->GetOutput()->GetNumberOfEdges())
    {
    ...
    std::cout << "nearest neighbor graph: " << nearestNeighborGraphFilter->GetOutput()->GetNumberOfEdges() << std::endl;
    return EXIT_FAILURE;
    }
